...
function initialize_quiver_plot(x_range, y_range, t, T, grid_points, vx_values, vy_values, x_start, y_start, x_finish, y_finish, x_curr, y_curr)
    ...
    quiver!(quiver_plot, [x_start], [y_start], quiver=([x_finish - x_start], [y_finish - y_start]), color=:green, linewidth=2)
...
function simulate(x_range, y_range, T)
    ...
    global x_start, y_start, x_finish, y_finish = -4.0, 28.0, 27.0, -9.0
    ...
    while abs(x_curr) < abs(x_finish) || abs(y_curr) < abs(y_finish)
        ...
        # Initialize plot
        quiver_plot = initialize_quiver_plot(x_range, y_range, t, T, grid_points, vx_values, vy_values, x_start, y_start, x_finish, y_finish, x_curr, y_curr)
